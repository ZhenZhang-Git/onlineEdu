...
import com.onlineEdu.content.feignClient.MediaServiceClient;
import com.onlineEdu.content.feignClient.SearchServiceClient;
...
import com.onlineEdu.content.model.po.CoursePublish;
import com.onlineEdu.content.model.po.CoursePublishPre;
...
public class CoursePublishServiceImpl implements CoursePublishService {
    ...
    public void commitAudit(Long companyId, Long courseId) {
        ...
        if ("202003".equals(auditStatus)) {
            OnlineEduException.cast("当前为等待审核状态，审核完成可以再次提交。");
        }
        //本机构只允许提交本机构的课程
        if (!courseBase.getCompanyId().equals(companyId)) {
            OnlineEduException.cast("不允许提交其它机构的课程。");
        }
        //课程图片是否填写
        if (StringUtils.isEmpty(courseBase.getPic())) {
            OnlineEduException.cast("提交失败，请上传课程图片");
        }
        //查询课程计划信息
        List<TeachplanDto> teachplanTree = teachplanService.findTeachplanTree(courseId);
        if (teachplanTree.size() <= 0) {
            OnlineEduException.cast("提交失败，还没有添加课程计划");
        }
    ...
    public void publish(Long companyId, Long courseId) {
        ...
        if (coursePublishPre == null) {
            OnlineEduException.cast("请先提交课程审核，审核通过才可以发布");
        }
        //本机构只允许提交本机构的课程
        ...
        if (!"202004".equals(auditStatus)) {
            OnlineEduException.cast("操作失败，课程审核通过方可发布。");
        }
        //保存课程发布信息
    ...
    public void uploadCourseHtml(Long courseId, File file) {
        ...
        if (course == null) {
            OnlineEduException.cast("远程调用媒资服务上传文件失败");
        }
    }
    ...
    public Boolean saveCourseIndex(Long courseId) {
        ...
        if (!result) {
            OnlineEduException.cast("创建课程索引失败");
        }
        return result;
    }
    ...
    private void saveCoursePublishMessage(Long courseId) {
        ...
        if (mqMessage == null) {
            OnlineEduException.cast("添加消息记录失败");
        }
    }
}
